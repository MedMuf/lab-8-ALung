...
data.frame(rbind(map_chr(surveys, 
                         typeof))
           ) |>
setNames(names(surveys))
...
evals_factored <- bind_cols(map_at(evals, c("teacher_id",
                   "weekday",
                   "academic_degree",
                   "seniority",
                   "sex"),
...
evals |>
  distinct(teacher_id, sex, seniority, academic_degree) |>
  mutate(seniority_category = ifelse(seniority <= 4, "junior", "senior")) |>
  
  pivot_longer(cols = c(sex, seniority_category, academic_degree),
